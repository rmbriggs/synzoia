...
// Stub for the mini-leaderboard rail (steps/daily). Returns an empty
// leaderboard so the rail renders without interfering with feed assertions.
const LEADERBOARD_STUB = { date: '2026-01-01', total_steps: 0, participating_users: 0, leaderboard: [] };

/**
 * Build a fetch mock that routes by URL:
 *   - /api/steps/daily -> leaderboard stub (fresh Response each call)
 *   - everything else  -> the provided posts payload (fresh Response each call)
 */
function makeFetchMock(postsPayload: unknown, postsStatus = 200) {
  return vi.fn().mockImplementation((url: string) => {
    const str = typeof url === 'string' ? url : '';
    if (str.includes('/steps/daily')) {
      return Promise.resolve(jsonResponse(LEADERBOARD_STUB));
    }
    return Promise.resolve(jsonResponse(postsPayload, postsStatus));
  });
}
...
describe('Feed page (post stream)', () => {
  ...
  it('shows the empty state when no posts have been written', async () => {
    globalThis.fetch = makeFetchMock({ posts: [] });
  ...
  it('shows an error card with retry on failed fetch', async () => {
    globalThis.fetch = makeFetchMock('boom', 500);
